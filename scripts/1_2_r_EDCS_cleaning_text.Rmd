...
```{r}
getwd()
EDCS <- jsonlite::fromJSON("../data/EDCS_merged_cleaned_attrs_2022-09-12.json")
```
...
### Comparing the different output of the the two cleanings (Lat-Epig/scripted in R) for Conservative text
```{r}
EDCS_clean %>% 
...
EDCS_clean %>% 
  filter(inscription_conservative_cleaning != clean_text_conservative) %>% 
  select(`EDCS-ID`, inscription, inscription_conservative_cleaning, clean_text_conservative) %>% 
  nrow() -> conservative_cleaning_diff

# what's conservative cleaning difference rate in the entire dataset in %
conservative_cleaning_diff/(nrow(EDCS_clean)/100)
```


```{r}
EDCS_clean %>% 
  filter(`EDCS-ID`=="EDCS-29100274") %>% 
  select(inscription, inscription_conservative_cleaning,clean_text_conservative) %>% 
  unlist()
```


### Comparing the different output of the the two cleanings (Lat-Epig/scripted in R) for Interpretive text
```{r}
EDCS_clean %>% 
