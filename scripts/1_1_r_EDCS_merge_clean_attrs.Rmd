...
temp <- list.files(path= "../data/2022_04_allProvinces/", pattern = "*.tsv")
```

## Loading data from multiple TSV files

```{r}
setwd("~/Github/EDCS_ETL/data/2022_04_allProvinces")
...
"date_not_before"     - the earliest start of the dating interval of the inscription (*terminus post quem*), YYYY representing year, where minus values are years BC, and plus values are years AD, as provided by the EDCS (Datierung)
"date_not_after"      - the latest end date of the dating interval of the inscription (*terminus ante quem*), YYYY representing year, where minus values are years BC, and plus values are years AD, as provided by the EDCS (Datierung)
"status"          - status keywords defining the contents of the inscription, keywords of several categories are mixed into one, containing both inscription classification and information on status of people, or the execution and technique of inscription. Individual keywords separeted by semicolon, in Latin, as provided by the EDCS (Inschriftengattung / Personenstatus)   
"inscription"     -   text of an inscription including the Leiden Marka up and special EDCS markup, as published by the EDCS. 
"inscription_conservative_cleaning" - result of cleaning function embedded in the Lat/Epig software, producing a conservative version of the text of an inscription. The text is as close to the preserved state of the text, without restorations and expansions
...
"material"          - material of an inscriptions, as provided by the EDCS               
...
"extra_text"        - additional text available on the HTML page that does not belong to any of the other categories, as provided by the EDCS               
"extra_html"        - additional hyperlinks, as provided by the EDCS           
...
"province_list" - name of provinces of finer spatial resolution that fall under the more general Provinz attribute in EDCS, created by SDAM and presented as a list
...
write(EDCS_json, file="data/EDCS_merged_cleaned_attrs_2022-04-28.json")
request("EDCS_merged_cleaned_attrs_2022-04-28.json", path="/sharingout/648597@au.dk/SDAM_root/SDAM_data/EDCS/public",
