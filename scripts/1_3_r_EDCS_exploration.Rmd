...
### Example with inscriptions from Sicilia
```{r}
EDCS %>% 
  filter(province == "Sicilia" & !is.na(date_not_before) & !is.na(latitude)) -> sicilia
```
...
```
```{r}
sicilia$latitude <- as.numeric(sicilia$latitude)
sicilia$longitude <- as.numeric(sicilia$longitude)
```
...
# addPolylines(data = roads, color = "purple", weight = 2, opacity = 0.7) %>%
  setView( lng = 14.326743, lat = 37.897122, zoom = 6 ) %>%
...
```{r}
# unique place names
length(unique(unlist(EDCS$place)))
```
First 10 place names
```{r}
head(table(unlist(EDCS$place)), 10)
```
...
```{r}
heidelberg_data<- length(str_subset(EDCS$partner_link, "heidelberg"))
heidelberg_data
...
```{r}
length(str_subset(EDCS$partner_link, "P\\d{1,6}"))
```
...
# map_all   #uncomment if you want to show the map, it requires a powerful computer for the entire dataset
```
...
```{r}
EDCS %>%
  filter(!is.na(EDCS$date_not_before)) -> has_start_yr
...
```{r}
EDCS %>%
  filter(!is.na(EDCS$date_not_after)) -> has_end_yr
...
### Date Range
```{r}
has_end_yr$date_not_before <- as.numeric(has_end_yr$date_not_before)
has_end_yr$date_not_after <- as.numeric(has_end_yr$date_not_after)



has_end_yr<- has_end_yr %>% 
  mutate(date_interval = (date_not_before - date_not_after) * -1)
...
```{r}
EDCS$date_not_before <- as.numeric(EDCS$date_not_before)

summary(EDCS$date_not_before)

EDCS$date_not_after <- as.numeric(EDCS$date_not_after)

summary(EDCS$date_not_after)
```
### Both dates
```{r}
dated <- EDCS %>% 
  filter(!is.na(date_not_before) | !is.na(date_not_after))

dated
```

```{r}
nrow(dated)/(nrow(EDCS)/100)
```
